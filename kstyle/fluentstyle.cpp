...
#include "fluent.h"
#include "fluentanimations.h"
#include "fluentframeshadow.h"
#include "fluentmdiwindowshadow.h"
#include "fluentmnemonics.h"
#include "fluentpropertynames.h"
#include "fluentshadowhelper.h"
#include "fluentsplitterproxy.h"
#include "fluentstyleconfigdata.h"
#include "fluentwidgetexplorer.h"
#include "fluentwindowmanager.h"
#include "fluentblurhelper.h"
...
namespace FluentPrivate
{
    ...
    class TabBarData: public QObject
    {
        ...
        Fluent::WeakPointer<const QWidget> _tabBar;
    ...
    class ComboBoxItemDelegate: public QItemDelegate
    {
        ...
        explicit ComboBoxItemDelegate( QAbstractItemView* parent ):
            ...
            _itemMargin( Fluent::Metrics::ItemView_ItemMarginWidth )
        {}
        ...
        //* proxy
        Fluent::WeakPointer<QAbstractItemDelegate> _proxy;
...
namespace Fluent
{
    ...
    Style::Style():
        ...
        , _tabBarData( new FluentPrivate::TabBarData( this ) )
        #if FLUENT_HAVE_KSTYLE||FLUENT_USE_KDE4
        , SH_ArgbDndWindow( newStyleHint( QStringLiteral( "SH_ArgbDndWindow" ) ) )
        , CE_CapacityBar( newControlElement( QStringLiteral( "CE_CapacityBar" ) ) )
        #endif
    {

        // use DBus connection to update on fluent configuration change
        auto dbus = QDBusConnection::sessionBus();
        dbus.connect( QString(),
            QStringLiteral( "/FluentStyle" ),
            QStringLiteral( "org.kde.Fluent.Style" ),
            QStringLiteral( "reparseConfiguration" ), this, SLOT(configurationChanged()) );

        dbus.connect( QString(),
            QStringLiteral( "/FluentDecoration" ),
            QStringLiteral( "org.kde.Fluent.Style" ),
            QStringLiteral( "reparseConfiguration" ), this, SLOT(configurationChanged()) );
        #if !FLUENT_USE_KDE4
    ...
    void Style::polish( QWidget* widget )
    {
        ...
        if( qobject_cast<QScrollBar*>( widget ) )
        {
            ...
            if( !hasParent( widget, "QWebView" ) )
            {
                ...
                { itemView->setItemDelegate( new FluentPrivate::ComboBoxItemDelegate( itemView ) ); }
            }
    ...
    QSize Style::sizeFromContents( ContentsType element, const QStyleOption* option, const QSize& size, const QWidget* widget ) const
    {
        ...
        switch( element )
        {
            ...
            case CT_Menu: return menuSizeFromContents( option, size, widget );
    ...
    QRect Style::progressBarGrooveRect( const QStyleOption* option, const QWidget* widget ) const
    {
        ...
        #else
        const bool horizontal( FluentPrivate::isProgressBarHorizontal( progressBarOption ) );
        #endif
    ...
    //______________________________________________________________
    QSize Style::menuSizeFromContents( const QStyleOption*, const QSize& contentsSize, const QWidget* ) const
    {

        // return expandSize( contentsSize, 0, Metrics::Menu_MarginHeight );
        return contentsSize;
    ...
    bool Style::drawFrameTabWidgetPrimitive( const QStyleOption* option, QPainter* painter, const QWidget* widget ) const
    {
        ...
        const auto& palette( option->palette );
        const auto background( _helper->frameBackgroundColor( palette ) );
        const auto outline( _helper->frameTabOutlineColor( palette ) );
    ...
    bool Style::drawFrameTabBarBasePrimitive( const QStyleOption* option, QPainter* painter, const QWidget* ) const
    {
        ...
        const auto rect( option->rect );
        const auto outline( _helper->frameTabOutlineColor( option->palette ) );
    ...
    bool Style::drawPanelButtonCommandPrimitive( const QStyleOption* option, QPainter* painter, const QWidget* widget ) const
    {
        ...
        if( flat )
        {
            ...
            auto outline( _helper->buttonOutlineColor( palette, mouseOver, hasFocus, opacity, mode ) );
            ...
            if( enabled && buttonOption->features & QStyleOptionButton::DefaultButton )
            {
                ...
                outline = _helper->buttonOutlineColor( palette, mouseOver, true, opacity, mode );
            }
    ...
    bool Style::drawPanelMenuPrimitive( const QStyleOption* option, QPainter* painter, const QWidget* widget ) const
    {
        ...
        const auto& palette( option->palette );
        // const auto outline( _helper->frameOutlineColor( palette ) );
        const auto outline( palette.color( QPalette::Window ) );
        const bool hasAlpha( _helper->hasAlphaChannel( widget ) );
        ...
        */
        if( !menuItemOption->icon.isNull() )
        {
            ...
            if( !enabled )
            {
                ...
                // if( sunken ) iconMode = QIcon::Selected;
    ...
    bool Style::drawMenuItemControl( const QStyleOption* option, QPainter* painter, const QWidget* widget ) const
    {
        ...
        // copy rect and palette
        auto rect( option->rect );
        // rect.adjust( 0, Metrics::Menu_MarginHeight, 0, Metrics::Menu_MarginHeight );
        const auto& palette( option->palette );
        ...
        if( menuItemOption->menuItemType == QStyleOptionMenuItem::SubMenu )
        {
            ...
            // color
            QColor arrowColor;
            // if( selected || sunken ) arrowColor = palette.color( QPalette::HighlightedText );
        ...
        if( !menuItemOption->text.isEmpty() )
        {
            ...
            // color role
            // const QPalette::ColorRole role = ( selected || sunken ) ? QPalette::HighlightedText : QPalette::WindowText;
            const QPalette::ColorRole role = QPalette::WindowText;
    ...
    bool Style::drawScrollBarSliderControl( const QStyleOption* option, QPainter* painter, const QWidget* widget ) const
    {
        ...
        const qreal handleSize = StyleConfigData::animationsEnabled() ? (2 * (1 - grooveAnimationOpacity) + Metrics::ScrollBar_SliderWidth  * grooveAnimationOpacity) : (int)Metrics::ScrollBar_SliderWidth;
    ...
    bool Style::drawHeaderSectionControl( const QStyleOption* option, QPainter* painter, const QWidget* widget ) const
    {
        ...
        // fill
        const auto &normal = palette.color( QPalette::Window );
    ...
    bool Style::drawTabBarTabShapeControl( const QStyleOption* option, QPainter* painter, const QWidget* widget ) const
    {
        ...
        if( selected )
        {
            ...
            if( animated ) color = _helper->alphaColor( hover, opacity );
            else if( mouseOver ) color = hover;
            else color = QColor();
        ...
        // outline
        const auto outline( selected ? _helper->frameTabOutlineColor( palette ) : QColor() );
    ...
    bool Style::drawSliderComplexControl( const QStyleOptionComplex* option, QPainter* painter, const QWidget* widget ) const
    {
        ...
        if( sliderOption->subControls & SC_SliderHandle )
        {
            ...
            // render
            _helper->renderSliderHandle( painter, handleRect, background, shadow, sunken );
    ...
    {
        #if QT_VERSION >= 0x050000 && FLUENT_HAVE_QTQUICK
