...
extern "C"
{
    ...
    { return new Fluent::StyleConfig(parent); }
}
...
namespace Fluent
{
    ...
    void StyleConfig::save()
    {
        ...
        QScopedPointer<KConfig> kde4Config(StyleConfigData::self()->config()->copyTo(kde4ConfigDirPath+"/fluentrc", nullptr));
        kde4Config->sync();
        #endif

        // emit dbus signal
        QDBusMessage message( QDBusMessage::createSignal( QStringLiteral( "/FluentStyle" ),  QStringLiteral( "org.kde.Fluent.Style" ), QStringLiteral( "reparseConfiguration" ) ) );
